...
int main(int argc, char* argv[]) {
	...
	if (pFile.is_open()){
		...
		while (Theta <= Beta) {
			pFile << ",";
			pFile << fixed << setw(6) << setprecision(1) << Theta;
			Theta += ThetaStep;
		}
		pFile << endl;
		...
		while (z <= Zend) {
			...
			while (Alpha < AlphaEnd) {
				pFile << fixed << setw(6) << setprecision(2) << z;
				pFile << ",";
				pFile << fixed << setw(6) << setprecision(1) << Alpha;
				...
				while (Theta<= Beta) {
					...
					Theta += ThetaStep;
					pFile << ",";
					pFile << fixed << setw(6) << setprecision(1) << L;
					}
				pFile << endl;
				/*
				pFile <<  fixed << setw(6) << setprecision(2) << AttenuationSum ;
				if (i < 15) {
					i++;
					}
				else {
	...
	else cout << "Unable to open file 'output.txt'."<<endl;
	pFile.close();
	return 0;
}